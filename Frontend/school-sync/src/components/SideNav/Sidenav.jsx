...
export default function Sidenav() {
  ...
  return (
    <aside className={`h-screen pb-2 md:pb-4 overflow-hidden border-r shadow-sm ${isExpanded ? 'w-[50vw] md:w-[16vw]' : 'w-[10vw] md:w-[5vw]'}`}>
 <nav className='h-full flex flex-col bg-white'>
   <div className='p-3 pb-2 flex justify-between md:pr-4'>
     <div className='flex items-center md:w-[6vw]'>
      <img src={logo} className={`w-[20vw] sm:w-[6vw] md:w-[4vw] ${isExpanded ? 'block' : 'hidden'}`} alt=""/>
        <span className={`text-red no-underline text-[5vw] sm:text-[1.5rem] md:text-[1.2rem] w-[5vw] md:w-[4vw] text-sm pl-[3vw] sm:pl-[1vw] md:pl-[0.5vw] font-bold ${isExpanded ? 'block' : 'hidden'}`}>
          S<span className="text-[#263238]">chool <br />Sync</span>
        </span>
      </div>
      <button onClick={toggleSidebar} className="focus:outline-none bg-white cursor-pointer text-black md:pr-2">
        <HiMenu className="w-[5vw] sm:w-[1.5vw] md:w-[1.2vw] h-[5vw] sm:h-[1.5vw] md:h-[1.2vw] pt-3" />
      </button>
    </div>
    <div className={`flex flex-col flex-grow p-4 ${isExpanded ? 'w-[70vw] sm:w-[17vw]' : 'w-[10vw] sm:w-[3vw]'} border-r shadow-sm`}>
 <div className="mb-[10vh] sm:mb-[4vh] pt-3 md:gap-y-8">
